...
**Merge seam (Q validation, 2026-08-06):** `git diff feat/transcript-boundary feat/transcript-document --stat` shows **`MainWindow.swift` is the ONLY file both branches touch** — WP-1 touches only `MainWindow.swift`, `TranscriptBoundary.swift`, `FeatureFlags.swift`; WP-2 touches `MessageTemplate.swift` (161 ±), `MessageCanvas.swift` (61 ±), `MainWindow.swift` (38 ±), `ThemeManager.swift`, `Package.swift`. So merge risk is **real but localised to `MainWindow.swift`** — resolve there first. Record the merge commit in the B1/B2 evidence trail.
...
> **Q validation (2026-08-06):** the original spec cited "line 229-231" for `allowedSchemes` and "line 100" for `urlAttributes`; actual locations are `allowedSchemes` at **line 97** (not 229-231), `urlAttributes` at **line 100** (not 103), `isURLAllowed` at **line 263** (correct). Structural claim stands — the shared set feeds both attribute paths.
...
| B2I-3 | Bridge contract honest: exactly the 5 real events (`bcReady, bcLink, bcImage, bcLoadEarlier, bcCopyMessage`); no `bcHeight` | Bridge-surface test + code inspection. **Doc-fix (Q validation):** route plan `single-webview-transcript-plan.md` §4.5 incorrectly lists `bcPinned` (telemetry-only, no native UI) instead of `bcCopyMessage`; the route plan was speculative — WP-2's shipped contract (5 events, no `bcPinned`, `bcCopyMessage` at `TranscriptTemplate.html:579`) is the truth. Update §4.5 to drop `bcPinned` / add `bcCopyMessage` and note it in B2I evidence |
