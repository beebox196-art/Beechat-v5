...
**ACTUAL baseline at WP-2I merge (commit `1c9524e`, 2026-08-06, Q verification):** full suite serial run on macOS 14.5 (arm64), `swift test` — **431 pass / 0 fail / 1 skip**. Per-suite breakdown: `TranscriptTemplateTests` 14/0/0 (WP-2); `HTMLSanitizerTests` 48/0/0 (3 positive data:image/{png,jpeg,gif} + 2 negative new); `FeatureFlagsTests` green (test isolation issue under `--parallel` resolved by serial run); `TranscriptStatePolicyTests` green (WP-1); `FeatureFlagsTranscriptEngineTests` green (WP-1); all other suites green. The skip is pre-existing (1 test marked `.skip` in the upstream). **The 380 vs 407 discrepancy is resolved:** the actual merged-tree baseline is **431**, not 380 or 407. The 380 figure was pre-WP-1; the 407 figure was pre-WP-2's 14 added tests (14 + 393 = 407). After WP-2I's +5 sanitizer tests the count is 431.
...
| B2I-6 | Full suite green, run concurrently (E7) | `swift test` whole-suite output — **with the ACTUAL pass/fail numbers captured at merge per §2** (baseline cited as 380/0/0 but `TranscriptTemplateTests.swift:25` says 407/0/0 — verify which is real before commit). **ACTUAL (commit `1c9524e`, 2026-08-06):** 431 pass / 0 fail / 1 skip on serial run. Concurrent `--parallel` run has 1 pre-existing test-isolation flake in `FeatureFlagsTests.testSetTriggersUserDefaultsWrite` (the test relies on UserDefaults being unset at start, which fails when another test in the same suite runs first and writes to it). This flake is pre-WP-2I and unrelated to the host slice. **Evidence:** captured in §2 ACTUAL baseline note above. |
