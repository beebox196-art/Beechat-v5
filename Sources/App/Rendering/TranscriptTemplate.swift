...
<style>
  ...
  :root {
    ...
    --bc-font-scale: 1;
    /* --bc-bg resolves to the theme-injected --bc-bg-surface so the message
       window matches the sidebar exactly (Tweak 1, Aug 2026). The hardcoded
       fallback (#FFFFFF / #1F1F1F) only matters if the WebView is loaded
       without theme tokens — e.g. a debug page load or a standalone fixture. */
    --bc-bg:        var(--bc-bg-surface, #FFFFFF);
    --bc-text:      #212121;
    ...
    /* Tweak 3 (Aug 2026): gentle outline, not stark. 0 1px 2px with low alpha
       reads as a soft drop on the white surface — enough to lift the bubble
       off the sidebar without becoming visually loud. System bubbles still
       override to none (see .msg[data-role="system"] .bubble). */
    --bc-shadow-bubble: 0 1px 2px rgba(0,0,0,0.08);
  ...
  @media (prefers-color-scheme: dark) {
    :root {
      --bc-bg:       var(--bc-bg-surface, #1F1F1F);
      --bc-text:     #E0E0E0;
      ...
      /* Tweak 3 (Aug 2026): dark surface is much darker than light, so the
         same 0.08 alpha would read as invisible. Bump the alpha slightly and
         the y-offset by 1px so the outline is still gentle but actually
         visible. Still well under the "stark" threshold. */
      --bc-shadow-bubble: 0 1px 3px rgba(0,0,0,0.35);
    }
  }
  ...
  .bubble .time {
    /* Tweak 2 (Aug 2026): Telegram-style — small dim timestamp at the
       bottom-right INSIDE the bubble. Absolute positioning anchored to the
       bubble (the parent has position:relative). pointer-events:none lets
       text-selection drags pass through the time element to the message
       text underneath — preserves the FR-MULTICOPY A1/A4 selection contract.
       The extra padding-bottom on .bubble reserves space so the time doesn't
...
function buildMessage({ id, role, senderName, badge, timeLabel, html, timestamp, fold }) {
  ...
  if (timeLabel) {
    ...
    // Tweak 2 (Aug 2026): the time is appended INSIDE the bubble (was a
    // sibling) so the CSS can position it at the bottom-right corner,
    // Telegram-style. The seam contract is unchanged — this is pure DOM
    // placement, not pipeline logic.
